...
func (c *Client) setupChannel(
	...
	// TODO: The following will be done in a channel.NewParamsUnsafe after #204
	app, err := channel.AppFromDefinition(prop.AppDef)
	if err != nil {
		c.log.Panicf("setupChannel: creating app from supposedly valid address failed: %v", err)
	}
	params, err := channel.NewParams(
		prop.ChallengeDuration, parts, app, prop.Nonce,
	)
	if err != nil {
		c.log.Panicf("setupChannel: creating parameters on supposedly vaild data failed: %v", err)
	}
